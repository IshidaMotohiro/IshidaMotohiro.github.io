...
|year       |       fit|        lwr|        upr| 
|:----------|----------:|----------:|----------:|
|2014-01-01 |    -1495.0029| -1596.9629| -1493.0430|
|2015-01-01 |    -2233.9963| -2335.9562| -2232.0363| 
|2016-01-01 |    -1748.0015| -1849.9615| -1746.0416| 
|2017-01-01 |    -1971.0000| -2072.9600| -1969.0401| 
|2018-01-01 |    -2192.707| -3206.204| -1179.211| 
...
# estimate %>% tail
estimate %>% ggplot(aes(x = year, y = 転出超過)) + geom_point() + geom_line(aes(y = fit),alpha = .3) +
  geom_ribbon(aes(ymin= lwr, ymax = upr), alpha = .8)
library(ggfortify)
p_data <- autoplot(result_local_reg2$model$y, main ="転出数")
p_trend <- autoplot(result_local_reg2$alphahat[, "level"], main = "トレンド" )
