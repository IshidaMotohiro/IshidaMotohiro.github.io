...
```{r}
## https://www.e-stat.go.jp/stat-search/database?page=1&layout=datalist&toukei=00200523&tstat=000000070001&cycle=7&tclass1=000001040402&cycle_facet=tclass1&second2=1
## 男女別都道府県内移動者数，他都道府県からの転入者数及び他都道府県への転出者数
## 全国，都道府県，３大都市圏，21大都市 
## 平成26年〜平成30年（21大都市）20190131日登録
H26_30 <- paste0("http://api.e-stat.go.jp/rest/2.1/app/json/getStatsData?appId=",ids,"&lang=J&statsDataId=0003103378&metaGetFlg=Y&cntGetFlg=N&sectionHeaderFlg=1")
H26_30 <- GET(H26_30)
#######################
H26_30_values  <-  map_df( content(H26_30) $GET_STATS_DATA $STATISTICAL_DATA $DATA_INF $VALUE,   ~ .[c("@tab", "@cat01", "@cat02", "@area", "@time", "@unit", "$" )])
H26_30_values %>% NROW()

H26_30_values %>% filter(`@tab` == "04", `@area` == "36000") 
```
...
tokushima_q %>%
  ...
  scale_y_continuous(name = "転出超過", sec.axis =  sec_axis(~ rescale(., to = range(shinki_mean1$徳島)), name = "徳島・新規有効求人倍率"))+
   theme(axis.text = element_text(angle = 80))# + theme_gray(base_family = "HiraKakuPro-W3") 
# file
# 
...
osaka_q  %>%
  ...
  scale_y_continuous(name = "転出超過", sec.axis =  sec_axis(~ rescale(., to = range(shinki_osaka$大阪)), name = "大阪・新規有効求人倍率"))+
   theme(axis.text = element_text(angle = 80))# + theme_gray(base_family = "HiraKakuPro-W3") 
# # file
...
## 徳島と大阪の求人倍率差{#tokushima_osaka0}

```{r eval=TRUE}
shinki_all %>% dplyr::select(X__1, 大阪=`27大阪`,徳島=`36徳島`)  %>% mutate(year = year(X__1), 差 = 徳島 - 大阪)　%>% group_by(year) %>% summarise(徳島= mean(徳島), 大阪 = mean(大阪), 倍率差 = mean(差))  %>% mutate_at(c("徳島","大阪","倍率差"), round,3) %>% slice(42:46)%>% knitr::kable()#slice(50:55) %>% knitr::kable()
```

## 徳島と大阪の求人倍率差{#tokushima_osaka1}
...
sa %>%
  ...
  geom_line(aes(x = year, y = rescale(差, to = range(転出超過) ) ), col = "red") + 
  scale_y_continuous(name = "転出超過", sec.axis =  sec_axis(~ rescale(., to = range(shinki_sa$差)), name = "徳島・大阪の新規有効求人倍率の差"))+
...
```{r}
fitted2017 <- fitted(model_vars_sa)
pred2018 <- predict(model_vars_sa, n.ahead = 1)[[1]]$転出超過 %>% broom::tidy() %>% 
  add_column(年 = c("2018"), .before = "fcst")

# > fitted(model_vars_sa) %>% NROW() 
# [1] 39
# > sa_t %>% NROW()
# [1] 43 つまりp=4 次
```
...
|年   |      fcst|     lower|      upper|        CI|
|:----|---------:|---------:|----------:|---------:|
|<font color="#ff0000">2018</font> | <font color="#ff0000">-2543.248</font>| -3273.500| -1812.9961|  730.2521|
|2019 | -2385.839| -3386.186| -1385.4922| 1000.3469|
|2020 | -2587.760| -3833.220| -1342.3001| 1245.4599|
|2021 | -2470.419| -4077.731|  -863.1061| 1607.3125|
|2022 | -1991.261| -3836.271|  -146.2505| 1845.0101|
2018年の日本人転出超過数は<font color="#ff0000">-2290</font>
...
1. 増税やリーマンショックなどのイベントの取り込み
3. 状態空間モデルでの推定（状態と観測の分離）
...
|year       |       fit|        lwr|        upr| 
|:----------|----------:|----------:|----------:|
|2014 |    -1495.0029| -1596.9629| -1493.0430|
|2015 |    -2233.9963| -2335.9562| -2232.0363| 
|2016 |    -1748.0015| -1849.9615| -1746.0416| 
|2017 |    -1971.0000| -2072.9600| -1969.0401| 
|<font color="#ff0000">2018</font> |    <font color="#ff0000">-2192.707</font>| -3206.204| -1179.211| 
...
# build_trend_reg
# # 
build_local_reg2 <- SSModel(
    H = NA,
   sa$転出超過 ~ SSMtrend(degree = 1, Q = NA) + 
   SSMregression(~ sa$差) +flag,
...
fit_local_reg2 <- fitSSM(build_local_reg2, inits = c(1,1,1))

result_local_reg2 <- KFS(
  fit_local_reg2$model,
  filtering = c("state","mean"),
  smoothing = c("state", "mean")
)
result_local_reg2$model$H
result_local_reg2$model$Q

interval_local_coef <- predict(fit_local_reg2$model, states = "regression",
                         interval = "confidence", level = 0.95)

coef_data <- cbind(data.frame(year = 1975:2017),
                   as.data.frame(interval_local_coef))
# coef_data

# forecast_trend_reg2 <- predict(fit_trend_reg2$model, interval = "confidence", level = 0.95)
forecast_local_reg2 <- predict(fit_local_reg2$model, interval = "prediction", level = 0.95)
forecast_local_reg2 <- predict(fit_local_reg2$model, interval = "confidence", level = 0.95)
forecast_local_reg2
# estimate <- cbind(sa, data.frame(YEAR = 1975:2017),
#                    as.data.frame(forecast_trend_reg2))
estimate <- sa %>% bind_cols(as.data.frame(forecast_local_reg2))
# estimate <- estimate %>% mutate(lwr = lwr - 100, upr + 100)
# estimate %>% tail
estimate %>% ggplot(aes(x = year, y = 転出超過)) + geom_point() + geom_line(aes(y = fit),alpha = .9, col = "red") +  geom_ribbon(aes(ymin= lwr, ymax = upr), alpha = .3)
# 
# library(ggfortify)
# p_data <- autoplot(result_local_reg2$model$y, main ="転出数")
# p_local <- autoplot(result_local_reg2$alphahat[, "level"], main = "ローカルレベル" )
...
# new_d <- rep(-0.562, 3)
# sa_new <- data.frame(year = c(ymd("2018-01-01"), ymd("2019-01-01"), ymd("2020-01-01")),
#                     転出超過 = rep(NA, 3), 差 = rep(-0.562, 3))
# 
# new_d <- SSModel(formula = 
#   転出超過 ~ SSMtrend(degree = 2) + 
#     SSMregression(~ 差) + flag1,
#   data = sa_new)
# attr(fit_trend_reg$model, "k")
# attr(new_d, "k")

flag1 <- rep(FALSE, 3) ; flag2 <- c(TRUE, FALSE, FALSE)

new_d1 <- SSModel(formula = 
  転出超過 ~ SSMtrend(degree = 1) + 
    SSMregression(~ 差) + flag1,
  data = sa_new)

new_d2 <- SSModel(formula = 
  転出超過 ~ SSMtrend(degree = 1) + 
    SSMregression(~ 差) + flag2,
  data = sa_new)


#new_d <- SSModel(formula = 
#   rep(NA, 3) ~ SSMtrend(degree = 2) + SSMregression(~ rep(NA,3)) + flag1)
# + SSMcustom(k = fit_trend_reg$model$k, m = fit_trend_reg$model$m)

pred_d1 <- KFAS:::predict.SSModel(
  object = fit_trend_reg2$model,
  newdata = new_d1,
  interval = "prediction", 
  level=0.9)
pred_d1
...
## https://shumagit.github.io/myblog/2018/09/04/kfas-locallevel-with-expl/
mu <-  result_local_reg2$alphahat[, "level"]
mu.se <-  result_local_reg2$V[3,3,]

# フィルタ化推定量
mu_filter_kfas <- result_local_reg2$a["level"]
# 平滑化推状態
mu_smooth_kfas <- result_local_reg2$alphahat


df_filter <- data.frame(
  y         = sa$ 転出超過, 
  time      = 1975:2017,
  mu_filter = fitted(result_local_reg2)
)
...
## 消費税増税の影響{#localpredict1}

2019年の転出超過数予測

| 増税|       fit|       lwr|       upr|
|--:| ---------:|---------:|---------:|
|<font color="#ff0000">あり</font>|<font color="#ff0000">-2254.708</font>| -3305.244| -1204.173|
|なし| -2192.707| -3206.204| -1179.211|
